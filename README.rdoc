...
By default the starting of a worker is accomplished by running a system command: 
  "QUEUE=queue_name rake resque:work"
Where "queue_name" is whatever queue the job is running on

However, there is also the option to override the entire command with the system command of your choice using:
	
  class Job
    extend Resque::Plugins::Director
    direct :command_override => "./command_to_run"
    @queue = :test
    
    #rest of your Job class here
  end

<b>command_override</b>:: This will run exactly what you put in the command override as a system command to start a worker, allowing you to customize the starting of a worker fully.
