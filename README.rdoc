...
This would run the command: "cd /path/to/run/in" && VERBOSE=1 PIDFILE=pid QUEUE=test usr/bin/rake environment resque:work"

There is also the option to override the entire command with that of your choice using:

	class Job
	  extend Resque::Plugins::Director
	  direct :command_override => "run this command!"
	  @queue = :test
