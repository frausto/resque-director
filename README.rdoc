...
resque-director is mainly useful for when you are managing a large number of workers and don't want to waste resources keeping all of them waiting when they are not being used.
...
* If a max_worker is less than min_worker then the default for max_worker will be used. 
* If a min_workers is set less than 1 then it will set it to 0.
